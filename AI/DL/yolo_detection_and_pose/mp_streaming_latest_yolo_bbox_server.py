...
def getYOLOBox(img, yolo_model):

    box_results = yolo_model.predict(img, conf = 0.6, classes = 0, verbose = False, show = False)
...
while cap.isOpened():
    ...
    frame = cv2.resize(frame, (imgsize_x, imgsize_y)) # 이미지 사이즈 조정
    ...
    if yolo_activate_switch == True:
        box_results = model_yolo.predict(frame, conf = 0.6, classes = 0, verbose = False, show = False)
        box_results = box_results[0].boxes
