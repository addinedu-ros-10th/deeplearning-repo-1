...
cap = cv2.VideoCapture("./00007_H_A_FY_C1.mp4")
"""0번: 노트북 카메라, 1번~: 기타 카메라. 카메라 포트 확인 시에는 터미널에 "ls -l /dev/video*" 커맨드를 입력하여 확인. """

model_path = "./dataN_186_yb_sdj_v1.3.pt" # 학습모델 적용하는 위치
...
while cap.isOpened():
    ...
    if yolo_activate_switch == True:
        ...
        box_confidence = box_results.conf
        ...
        # cv2.putText(frame, "Human " + str(round(box_confidence[0].item(), 2)), (x1+10, y1+20), cv2.FONT_HERSHEY_COMPLEX, 0.7, (100, 100, 0))
