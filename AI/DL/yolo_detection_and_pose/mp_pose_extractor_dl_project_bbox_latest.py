...
local_base_link = "SY"                                                                # 영상 모음 폴더
detection_training_data = pd.read_csv("Fall Detectoin Training Data Set - 정광민.csv") # 데이터셋
...
def poseDataExtractor(cap_link, warning_start_frame, fall_start_frame):
    ...
    size_x = cap.get(cv2.CAP_PROP_FRAME_WIDTH)
    size_y = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
    ...
    while True:
        ...
        box_results = model_yolo.predict(frame, classes = 0, verbose = False, show = False)
        ...
        for idx, _ in enumerate(box_class):
        ...
        if x1 > 0 and y1 > 0 and x2 > 0 and y2 > 0:
            ...
            if x_width % 2 == 1:
                x_center = int((x1 + x2)/2) + 1
            else:
                x_center = (x1 + x2)/2

            if y_width % 2 == 1:
                y_center = int((y1 + y2)/2) + 1
            else:
                y_center = (y1 + y2)/2

            x_min = x_center - (constant_x/2)
            x_max = x_center + (constant_x/2)
            y_min = y_center - (constant_y/2)
            y_max = y_center + (constant_y/2)

            x_min, x_max, y_min, y_max = int(x_min), int(x_max), int(y_min), int(y_max)

            print(x_min, x_max, y_min, y_max)

            # if x_min > 0 and x_max < resize_x and y_min > 0 and y_max < resize_y:
            if x_min > 0 and x_max < size_x and y_min > 0 and y_max < size_y:
        ...
        cv2.imshow("image extracting", frame) # 이미지 출력
